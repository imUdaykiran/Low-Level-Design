...
public class Main {
    public static void main(String args[]) {
        MovieTicketBooking bookingSystem = MovieTicketBooking.getInstance();
        ...
        // Add shows
        Show show1 = new Show("S1", movie1, theater1, LocalDateTime.now(), createSeats(10, 10));
        Show show2 = new Show("S2", movie2, theater2, LocalDateTime.now(), createSeats(8, 8));
    ...
    private static Map<String, Seat> createSeats(int rows, int columns) {
        ...
        for (int row = 1; row <= rows; row++) {
            for (int col = 1; col <= columns; col++) {
                ...
                Seat seat = new Seat(seatId, row, col, seatType, SeatStatus.AVAILABLE, price);
